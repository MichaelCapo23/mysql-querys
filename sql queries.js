...
SELECT * FROM `purchases` AS p WHERE MONTH(P.`added`) > 2 AND p.`price` > 200 ORDER BY p.`price` ASC
...
SELECT p.*, i.`name`, i.`price` FROM `purchases` AS p LEFT JOIN `items` AS i ON p.`customers_id`= c.`ID`
//    Select all purchases, group them by the customer that bought them
SELECT p.*, CONCAT(c.`family_name`, ', ', c.`given_name`) FROM `purchases` AS p LEFT JOIN `customers` AS c ON p.`customers_id`= c.`ID`
...
SELECT SUM(`price`), CONCAT(`customers`.`family_name`, ', ', `customers`.`given_name`) FROM `purchases` AS p join `customers` GROUP BY `customers_id`
//    Select the sum of all the purchases, group them by each month (look up mysql Month and Year)
SELECT SUM(p.`price`), YEAR(p.added) AS `year`, MONTH(p.added) AS `month`
FROM `purchases` AS p join `customers` GROUP BY YEAR(p.added), MONTH(p.added)
